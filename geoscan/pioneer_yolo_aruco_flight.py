...
# Путь к обученным весам YOLO.
# Если файл лежит рядом со скриптом, достаточно имени файла.
YOLO_MODEL_PATH = "yolo11n_two_boats_best.pt"

# Минимальная уверенность детекции.
YOLO_CONF = 0.45

# Размер входного изображения YOLO. Уменьшите до 416/320,
# если Pioneer не успевает обрабатывать поток в реальном времени.
YOLO_IMGSZ = 640
...
def video_worker():
    ...
    try:
        ...
        print("[VIDEO] Загрузка YOLO...")
        yolo_model = YOLO(YOLO_MODEL_PATH)
        print(f"[VIDEO] YOLO загружена: {YOLO_MODEL_PATH}")
        ...
        while running:
            ...
            try:
                ...
                # Количество найденных кораблей
                cv2.putText(
                    frame,
                    f"BOATS: {detected_boats}",
                    (20, 135),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.65,
                    (255, 0, 255),
